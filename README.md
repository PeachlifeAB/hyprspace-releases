# Hyprspace Releases

This repository is the public release surface for Hyprspace.

- Source code repository: (`PeachlifeAB/hyprspace-core`)
- Public Homebrew tap: https://github.com/PeachlifeAB/homebrew-tap
- Legal disclosure: ./LEGAL.md
- License: ./LICENSE

## Install with Homebrew

```bash
brew tap PeachlifeAB/tap
brew install --cask hyprspace
```

## Manual download

Versioned release zips are published on the GitHub Releases page for this repository. Each zip contains the app bundle, CLI, completions, configuration docs, legal notices, and bundled support assets needed for offline inspection.

## Notes

Hyprspace releases are code-signed. See `LEGAL.md` for the current disclosure and audit-facing details.
